...
Take a moment to browse [asn_games.csv](asn_games.csv). Each line contains
information about one game, formatted as four [comma separated values
...
> **Note** On many computers, files that end with `.csv` are, by default, open
> in Microsoft Excel. That's actually a pretty convenient way to view and
> process the data. However, [asn_games.csv](asn_games.csv) is just a plain
> text file. Depending upon your version of Mu, you may, or may not, be able
> to open a `.csv`. To view it as a plain text file, you could open it another text editor (e.g. Notepad on
> Windows, or TextEdit on a Mac), or try making a copy 
...
Write a function called `get_num_lines(fname)` that returns (*not* prints!)
the number of lines in a game
...
formatted as in [asn_games.csv](asn_games.csv) into a list of values. This
will be very useful in the following questions.
...
In this example a single game has the most recommendations, but in general
there could be multiple games that tie for the most.

### Part 3.b

Write a function called `print_highest_price(fname)` that prints (not
returns!) the game(s) with the highest price, formatted like this:
...
In this example a single game has the highest price, but in general there
could be multiple games that tie for highest price.
...
*dictionary* of all the game names in `fname`. The dictionary keys are the
names of the games, and the corresponding values are the number of times that
name appears as the name of game. Make sure there are no extra whitespace
...
different than the example. It's okay if yours are in a different order, as
long as all the same keys and values are the same.
...
*numbered*, *sorted alphabetically*, and also *show how many times they occur
in the file*. Format the output as shown.
...
[histogram](https://en.wikipedia.org/wiki/Histogram) of the metacritic scores
in `fname`.

The metacritic scores range from 0 to 100. For this question, **ignore all
games that have a metacritic score of 0** (don't include them in the
histogram).

Also, **don't remove duplicate games**. Leave them in. This might make the
data a little less accurate, but it simplifies the question.
...
As in the example below, at the top of each rectangle write the number of
games. Underneath the entire histogram, put a label saying what file the data
is from. Use the
